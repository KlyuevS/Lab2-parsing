...
CPE: в MITRE CVE API для Tomcat нет готового поля `cpes` для большинства записей, поэтому CPE формируется из API-полей `affected.vendor`, `affected.product` и `affected.versions` в формате CPE 2.3. Диапазоны версий разворачиваются в список конкретных CPE. Например: `cpe:2.3:a:apache:tomcat:9.0.92:*:*:*:*:*:*:*`. Для milestone-версий используется поле update, например `cpe:2.3:a:apache:tomcat:9.0.0:milestone7:*:*:*:*:*:*`.
...
Сложность: в CVE API не для всех старых записей есть CVSS и CWE. Код сохраняет то, что есть в MITRE API, без подстановки выдуманных значений. В текущем запуске получено 113 CVE; CPE с конкретными версиями есть у всех записей, у 59 записей нет CVSS, у 47 записей нет CWE.
